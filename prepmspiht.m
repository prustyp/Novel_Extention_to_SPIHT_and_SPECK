...
X1=imread('D:\priya\project-spiht\standard_test_images\lena_gray_512.tif');
figure(1)

createfigure2(X1)
org=double(X1);
n_max = (abs(max(max(org)')))
nn=(1/2)*n_max;
image=org-nn;
...
createfigure2(uint8(re1))

 Q = 255;


    PSNR=psnr(X1,uint8(re1));
...
K = [0.05 0.05];
  window = ones(8);
  L = 100;
[mssim, ssim_map] = ssim_index(X1,xx , K, window, L);
qlms=mssim
Z=[Z qlms]
